...
class FakeH264VideoEncoder extends Emitter
{
	...
	constructor(params)
	{
		...
		super();
		//Create code
		const encoder = new Native.FakeH264VideoEncoderWorkerFacade();
		//Set bitrate
	...
	{
		//Create the source
		const bridge = new Native.MediaFrameListenerBridge(this.encoder.GetTimeService(), this.lfsr.seq(31), !nosmooth);
		//Add track
		const track = new VideoEncoderIncomingStreamTrack(trackId,this.encoder,bridge); 
		//Add it to the encoder
		this.encoder.AddListener(Native.MediaFrameListenerBridgeToMediaFrameListener(bridge));
		//Add listener
		track.once("stopped",(track)=>{
			//Remove source from listener
			this.encoder.RemoveListener(Native.MediaFrameListenerBridgeToMediaFrameListener(bridge));
			//Stop bridge
			bridge.Stop();
			//Remove from map
